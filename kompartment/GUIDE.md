...
**Apply** follows the switch too: with auto-run on, the run starts once the
edited model is in, and with it off, nothing runs until you ask.
...
1. **New** — an empty canvas with the simulation settings filled in, which is
   also what the page opens on. It starts empty on purpose: a worked model is
   somebody else's, and the first thing to do with one is delete it. The picker beside the button has nine if you want
...
**Several can be open at once.** A block's settings are a window rather than a
dialog: the page behind stays usable, so double-clicking a second block — on
the diagram, in the tree, in the matrix — opens its settings beside the first,
a step down and to the right, and a value can be read off one and typed into
the other. Press a window to bring it to the front; Escape closes the one the
keyboard is in. Asking for a block whose window is open already brings that
window forward instead of opening a second copy. The other dialogs — Save, the
probabilistic run, the distribution editor — are still dialogs, and while one
is open the windows wait behind it.

**What is wrong with a block is said at the top of its settings**: the faults
in the problem strip that name it, including one that stops the model being
built at all — an equation indexed by a list its block cannot reach, say. That
kind used to surface only when a run failed, and then only in the strip. It is
found now by the build that works out the values at the start, as soon as the
edit is made — for any model small enough to build in under half a second;
larger ones find it when they are run — and it is marked on the block in the
tree and on the diagram as well, and said in words: *Canisters’s matrix
degradation rate*, not the name the builder gives the setting inside.
...
| **Matrix degradation rate** (per index) | The fraction of the exposed waste form dissolving per unit time, which carries everything bound in it out **congruently** — every nuclide in proportion to what the matrix holds. One number serves every index; where the block is indexed by more than the nuclides — a waste type, say, with fuel and metal parts dissolving at very different rates — each index can have its own, under **Values per index** or from an indexed parameter. A rate per nuclide is allowed too, and makes the release incongruent |
...
### Editing it in the JSON tab

The **JSON** tab shows the model as this text, and the text can be edited
there. **Apply** is offered once it differs from the model, and puts it in as
the model in one step, which Undo takes back; with auto-run on, the run
follows. The text is then written out again from the model, because the model
may hold it differently from how it was typed: units that are worked out are
worked out again, and shorthands are spelt in full.

It is checked after each pause in typing. Where it is not JSON, the line under
the toolbar says where and what is wrong — a comma after the last item, a
property name without its double quotes, a string left open — and **Show me**
puts the caret on the spot. Where it is JSON but not a model this tool can
read, the line says why. Apply waits until either is put right.

**Colour the syntax** paints property names, strings, numbers and `true`,
`false` and `null` each in a colour of their own. It is a copy of the text
drawn under the box you type in, which is how a plain text box can be coloured
while keeping its own undo, selection and caret. Only the lines in view are
