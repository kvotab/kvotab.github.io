 ...
 * A tool that can only write CSV is one whose results cannot be looked at
 * the way everyone looks at results.
 *
 * WHY IT IS WRITTEN OUT RATHER THAN LINKED. The usual way is to call the HDF5
 * library; the browser equivalent is h5wasm, which is 4.7 MB of WebAssembly
 * fetched from a CDN. This tool has no build step, no dependencies and no
...
export const F64 = { kind: 'float', size: 8, exp: 11, expLoc: 52, mant: 52, bias: 1023 };
/** The same, half the size, for a series that does not need the digits. */
export const F32 = { kind: 'float', size: 4, exp: 8, expLoc: 23, mant: 23, bias: 127 };
/** A signed 32-bit integer: counts. */
export const I32 = { kind: 'int', size: 4, signed: true };
/** A variable-length UTF-8 string, held in the global heap. */
export const STR = { kind: 'vlenstr', size: 16 };
 ...
 */
export function dataset(data, dt = F64, attrs = {}, dims = null) {
	if (dims) {
 ...
 */
export function put(root, path, node) {
 ...
 */
export function lookup3(u8, from = 0, length = u8.length - from) {
