/**
 * A sparse LU of I - h*J that keeps its pivots from one factorisation to the
 * next.
 *
 ...
 *
 * This is a port of the same class in resources/js/facsimile-solver.js, where
 * the canister and reactive-transport pages use it; the measurements behind
 * the constants are recorded there and in INTERNALS.md.
 */
...
export class RefactorLU {
	...
	/** I - h*J into V; false if a value is not finite. */
	load(h, values) {
		const { n, V, jSlot, dSlot } = this;
		...
		for (let i = 0; i < n; i++) V[dSlot[i]] += 1;
		return finite;
	}

	/** Factorises I - h*J; false when it cannot be done with pivots that pass the test. */
	factor(h, values) {
		if (!this.valid) this.nslots = this.nbase;
		if (!this.load(h, values)) {
			this.valid = false;
			return false;
		}
		let from = 0;
	...
	/** Solves (I - h*J) x = b. Returns a new Float64Array unless `out` is given. */
	solve(b, out) {
