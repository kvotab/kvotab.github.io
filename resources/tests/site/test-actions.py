...
PROBE = r"""(async () => {
  const results = [];
  // Every (element, event) pair declared anywhere on the page.
  const EVENTS = ['click', 'change', 'input', 'keyup', 'keydown', 'blur', 'focus'];
  const bindings = [];
  for (const on of EVENTS) {
    for (const el of document.querySelectorAll('[data-on-' + on + ']')) {
      bindings.push({ el, on, name: el.getAttribute('data-on-' + on) });
    }
  }
  const elements = bindings;

  for (const { el, on, name } of bindings) {

    // These have no same-named global; they open a hidden file input.
    const PICKERS = { openFilePicker: 'fileInput', bulkOpenFilePicker: 'bulk_file' };
    if (PICKERS[name]) {
      let opened = 0;
      const input = document.getElementById(PICKERS[name]);
  ...
  const checkbox = document.getElementById('showTotal') || document.getElementById('bulk_file');
  const originalToggle = window.toggleShowTotal;
  let checkboxCalls = 0;
  if (checkbox && originalToggle) {
    ...
    checkboxCalls = 1;                    // not applicable on this page
  }
  ...
  const throwTarget = window.applySelectedPreset ? 'applySelectedPreset' : 'bulkClearResults';
  const throwElement = document.getElementById('presetSelect')
    || document.querySelector('[data-on-change="bulkClearResults"]');
  const origApply = window[throwTarget];
  window[throwTarget] = function () { throw new Error('deliberate test failure'); };
  const thrownErrors = [];
  const origError2 = console.error;
  console.error = (...args) => { thrownErrors.push(args.map(String).join(' ')); };
  throwElement.dispatchEvent(new Event('change', { bubbles: true }));
  await new Promise(r => setTimeout(r, 60));
  console.error = origError2;
  window[throwTarget] = origApply;
...
async def main():
    ...
    async with websockets.connect(ver['webSocketDebuggerUrl'], max_size=200 * 1024 * 1024) as bws:
        url = 'http://127.0.0.1:8765/' + (sys.argv[1] if len(sys.argv) > 1 else 'rb.html')
        tid, page = await open_page(bws, url=url, settle=8)
