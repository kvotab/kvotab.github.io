...
| `test-ui.py` | the page in Chrome: loads the fixtures, analyses, decides, saves, then takes the saved `.docx` apart (see below); on the way, the (i) beside each section, setting and tab -- every slot has a topic and every "Read more in Help" a target (`KvotInfo.audit()`), each (i) opens its own topic, the ×, the same (i) and Escape close it, a list of choices marks the current setting, the panel sits between header and footer, and no `title` tooltip is left | Python `websockets`, Chrome |
