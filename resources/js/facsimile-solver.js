...
(function (root, factory) {
  ...
  class SparseLU {
    ...
    _spsolve(Ap, Ai, Ax, col) {
      ...
      for (let px = top; px < n; px++) {
        ...
        const J = pinv[j];
        if (J < 0) continue;
        // Counted whatever the value: a zero here at the trial is rarely a
        // zero at every step, and the count is compared with RefactorLU's,
        // which is structural too.
    ...
    factorize(A, opts = {}) {
      ...
      this.singular = false;
      this.failColumn = -1;
      this.flops = 0;                  // multiply-adds, for comparing with RefactorLU
      this.pinv.fill(-1);
      this.mark.fill(-1);
      this.stampCounter = 0;
      this.x.fill(0);
  ...
  /* ======================================================================
     3b. Sparse LU that keeps its pivots between factorisations
     ====================================================================== */
  /*
    The iteration matrix is factorised again and again in a run: its pattern
    never changes and its values change slowly. A dense or a searching sparse
    LU re-derives everything each time. On the canister model that is 85,000
    multiply-adds for 64 unknowns, where an elimination in a good order needs
    about 6,000. On a reactive-transport model of 350 unknowns the sparse
    Gilbert-Peierls LU spent 84 % of the run choosing and searching.

    So the pivots are chosen once from the values, by threshold Markowitz: at
    each step, the entry with the least (row count - 1) x (column count - 1),
    among those at least PIVOT_THRESHOLD times the largest in their column.
    The search looks at the SEARCH_COLUMNS columns with the fewest entries
    that have a candidate, which is Zlatev's restricted search. Four columns
    left the transport model's factor a third larger than sixteen. Every elimination is recorded, and each later
    matrix is eliminated by replaying the record over the same structure, with
    nothing searched.

    Every step is checked again as it is replayed. The pivot must be nonzero
    and finite, and no entry below it may exceed it by more than
    1/KEEP_THRESHOLD, which bounds how much any entry can grow in that step.
    The choice is made at 0.1 (UMFPACK's and MA48's default) and the pivots
    are kept while 0.01 holds (MA57's default). A pivot chosen at the
    stricter test can drift a long way before it fails the looser one. On a
    reactive-transport model of 350 unknowns, re-checking at 0.1 meant
    choosing again at half of all factorisations; at 0.01 it is one in twenty,
    and on the canister model one in twenty-five. A step that fails is chosen
    again from the matrix as it stands, and so is every step after it. The
    steps before passed and are kept.

    What cannot be factored this way, because nothing is left to pivot on or a
    value is not finite, is declined, and the caller hands it to the LU it
    would otherwise have used. That LU either factors it or reports it
    singular exactly as before. Nothing is ever solved with a pivot that
    failed the test.

    Why not pivots fixed once from the pattern alone, as chemistry
    preprocessors write them? Measured on the canister model's own iteration
    matrices, which are scaled by the size of each species, 1,922 of 1,971
    failed this test on the diagonal.

    Storage is by slot: every entry of the matrix and of its fill has a place
    in one array V, and the slots are numbered in the order they are made
    (the pattern first, then each step's fill). A step chosen again therefore
    discards exactly the slots made after it, and memory is the size of the
    factor, not n x n.
  */
  ...
  // and the searching LU, which stores only the factor, is left the job.
  const OPS_BUDGET = 4e6;

  /** A growable Int32Array or Float64Array: returns `a`, or a copy with room for `need`. */
  ...
  class RefactorLU {
     ...
     * @param {Int32Array|null} [columns] a fixed column order: then only the
     *   row of each pivot is chosen, by the same test, preferring short rows.
     *   Without it, both are chosen by threshold Markowitz.
     */
    constructor(pattern, columns = null) {
      ...
      for (let j = 0; j < n; j++) {
        for (let p = pattern.colPtr[j]; p < pattern.colPtr[j + 1]; p++) {
          const i = pattern.rowIdx[p];
          this.slotRow[ns] = i; this.slotCol[ns] = j;
          this.jSlot[p] = ns; mark[i] = ns; ns++;
        }
        if (mark[j] >= 0 && this.slotRow[mark[j]] === j && this.slotCol[mark[j]] === j) this.dSlot[j] = mark[j];
        else { this.slotRow[ns] = j; this.slotCol[ns] = j; this.dSlot[j] = ns; ns++; }
      ...
      // column below is L entries Lstart[s].., its row to the right U entries
      // Ustart[s].., and its |L| x |U| updates write the slots T[Tstart[s]..].
      // fillStart[s] is the slot count before step s made any fill.
      this.pr = new Int32Array(n); this.pc = new Int32Array(n); this.pslot = new Int32Array(n);
      this.Lstart = new Int32Array(n + 1); this.Ustart = new Int32Array(n + 1);
      this.Tstart = new Int32Array(n + 1); this.fillStart = new Int32Array(n + 1);
      this.Lslot = new Int32Array(64); this.Lrow = new Int32Array(64);
      this.Uslot = new Int32Array(64); this.Ucol = new Int32Array(64);
      this.T = new Int32Array(64);
      this.valid = false;
      this.ops = 0;                    // multiply-adds per factorisation
      this.nnz = 0;                    // entries of L + U
      this.repivots = 0;               // times pivots were chosen again
      ...
      this.rowActive = new Uint8Array(n); this.colActive = new Uint8Array(n);
      this.rc = new Int32Array(n); this.cc = new Int32Array(n);
      this.head = new Int32Array(n + 2); this.next = new Int32Array(n); this.prev = new Int32Array(n);
      this.at = new Int32Array(n); this.atStamp = new Int32Array(n); this.stamp = 0;
      this.y = new Float64Array(n);
    }

    /** Mass - a*J, scaled by w as in DenseLU, into V; false if a value is not finite. */
    ...
    /** Factors Mass - a*J; false when it cannot be done with pivots that pass the test. */
    factor(a, values, w, mass) {
      if (!this.valid) this.nslots = this.nbase;
      if (!this.load(a, values, w, mass)) { this.valid = false; return false; }
      let from = 0;
      if (this.valid) {
        from = this.refactor();
      ...
      this.repivots++;
      if (!this.choose(from)) { this.valid = false; return false; }
      // Choosing from the middle keeps pivots chosen for an earlier matrix,
      // and the order can drift: on the transport model, re-choosing from the
      // failing step every time took the work per factorisation from 190,000
      // multiply-adds to 820,000. Past REORDER_GROWTH times the last choice
      // made from the start, it is made from the start again.
      if (from === 0) this.fullOps = this.ops;
      else if (this.ops > REORDER_GROWTH * this.fullOps) {
        this.load(a, values, w, mass);
        if (!this.choose(0)) { this.valid = false; return false; }
    ...
    _bucketOut(j) {
      ...
      if (prev[j] >= 0) next[prev[j]] = next[j]; else head[cc[j]] = next[j];
    ...
    _newSlot(i, j) {
      const s = this.nslots++;
      this.V[s] = 0; this.slotRow[s] = i; this.slotCol[s] = j;
      this.rowSlots[i].push(s); this.colSlots[j].push(s);
      this.rc[i]++; this.cc[j]++;
      return s;
    }

    /**
     * Chooses the pivots of steps s0.. by threshold Markowitz, from V as the
     * kept steps before s0 left it, and eliminates as it goes. False when no
     * acceptable pivot remains or a value is not finite.
     */
    choose(s0) {
      const { n, pr, pc, pslot, Lstart, Ustart, Tstart, fillStart, rowSlots, colSlots, rowActive, colActive, rc, cc, head, at, atStamp } = this;
      // Keep the pattern and the fill of the kept steps; the rest goes.
      this.nslots = s0 > 0 ? fillStart[s0] : this.nbase;
      rowActive.fill(1); colActive.fill(1);
      for (let s = 0; s < s0; s++) { rowActive[pr[s]] = 0; colActive[pc[s]] = 0; }
      for (let i = 0; i < n; i++) { rowSlots[i].length = 0; colSlots[i].length = 0; }
      rc.fill(0); cc.fill(0);
      {
        const { slotRow, slotCol } = this;
        for (let s = 0; s < this.nslots; s++) {
          const i = slotRow[s], j = slotCol[s];
          if (!rowActive[i] || !colActive[j]) continue;
          rowSlots[i].push(s); colSlots[j].push(s);
          rc[i]++; cc[j]++;
        }
      }
      head.fill(-1);
      for (let j = 0; j < n; j++) if (colActive[j]) this._bucketIn(j);
      atStamp.fill(0); this.stamp = 0;
      ...
      for (let s = s0; s < n; s++) {
        ...
        for (let c = 1, cmax = fixed >= 0 ? 1 : n - s; c <= cmax && examined < SEARCH_COLUMNS && best > 0; c++) {
          for (let j = fixed >= 0 ? fixed : head[c]; j >= 0 && examined < SEARCH_COLUMNS && best > 0; j = fixed >= 0 ? -1 : this.next[j]) {
            ...
            for (let k = 0; k < list.length; k++) {
              ...
              const v = Math.abs(V[sl]);
              if (v < thr) continue;
              const cost = (rc[i] - 1) * cj, ratio = v / m;
              if (cost < best || (cost === best && ratio > bestRatio)) { best = cost; bestRatio = ratio; bslot = sl; }
            }
          }
        }
        if (bslot < 0) return false;
        const p = slotRow[bslot], q = this.slotCol[bslot];
        pr[s] = p; pc[s] = q; pslot[s] = bslot; fillStart[s] = this.nslots;
        rowActive[p] = 0; colActive[q] = 0;
        this._bucketOut(q);
        // Its column below and its row to the right, among what is active.
        const Lcol = colSlots[q], Urow = rowSlots[p];
        this.Lslot = room(this.Lslot, lp + Lcol.length); this.Lrow = room(this.Lrow, lp + Lcol.length);
        this.Uslot = room(this.Uslot, up + Urow.length); this.Ucol = room(this.Ucol, up + Urow.length);
        const { Lslot, Lrow, Uslot, Ucol } = this;
        Lstart[s] = lp; Ustart[s] = up; Tstart[s] = tp;
        for (let k = 0; k < Lcol.length; k++) { const sl = Lcol[k], i = slotRow[sl]; if (rowActive[i]) { Lslot[lp] = sl; Lrow[lp] = i; lp++; } }
        for (let k = 0; k < Urow.length; k++) { const sl = Urow[k], j = this.slotCol[sl]; if (colActive[j]) { Uslot[up] = sl; Ucol[up] = j; up++; } }
        const l0 = Lstart[s], u0 = Ustart[s];
        const most = (lp - l0) * (up - u0);
        this.T = room(this.T, tp + most);
        this.V = room(this.V, this.nslots + most); this.slotRow = room(this.slotRow, this.nslots + most); this.slotCol = room(this.slotCol, this.nslots + most);
        ...
        for (let k = l0; k < lp; k++) {
          ...
          for (let r = 0; r < row.length; r++) {
            ...
            row[keep++] = sl; at[j] = sl; atStamp[j] = stamp;
          }
          row.length = keep;
          for (let u = u0; u < up; u++) {
            const j = Ucol[u];
            // Fill is structure whatever its value now: a later matrix may
            // put a number there.
        ...
        for (let u = u0; u < up; u++) { const j = Ucol[u]; cc[j]--; this._bucketIn(j); }
        if (tp > this.limit) return false;
      }
      Lstart[n] = lp; Ustart[n] = up; Tstart[n] = tp; fillStart[n] = this.nslots;
      this.ops = tp;
    ...
    solve(b, out) {
      const { n, V, pr, pc, pslot, Lstart, Ustart, Lslot, Lrow, Uslot, Ucol, y } = this;
  ...
  function makeIterationMatrix(pattern, values, mode = 'auto', mass = null) {
    ...
    // never touches.
    let denseLU = null;
    const dense = () => denseLU || (denseLU = new DenseLU(n));
    ...
    const denseForm = (a, current) => {
      dense().formAndFactor(a, pattern, current, wScale, mass);
      if (denseLU.singular) throw new Error(singularMessage(mass, denseLU.failColumn));
      return denseLU;
    };
    if (mode === 'dense') {
      info.lu = 'dense';
      ...
      return {
        info,
        form(a, current, w) { keepScale(w); lu = denseForm(a, current); },
        solve(rhs, out) { return scaledSolve(lu, rhs, out); },
      };
    }
    ...
    // Past a third of a dense factor the searching sparse factorisation is
    // slower than the dense one, so that is where the dense LU would be used.
    const fillsIn = best > 0.35 * n * n;
    // The sparse LU that keeps its pivots (3b). A factorisation it declines
    // goes to the LU that would otherwise have been used, whose answer, or
    // singular message, is final.
    const kept = (lu) => {
      const fallback = fillsIn ? denseForm : sparseForm;
      let current = lu;
      const trialRepivots = lu.repivots;
      const ordering = lu.columns ? `${q ? 'reverse Cuthill-McKee' : 'natural'} columns, threshold rows` : 'threshold Markowitz';
      Object.assign(info, { sparse: true, lu: 'refactor', ordering, fill: lu.valid ? lu.nnz : null, repivots: 0, fallbacks: 0 });
      return {
        info,
        form(a, J, w) {
          keepScale(w);
          if (lu.factor(a, J, wScale, mass)) {
            current = lu;
            info.fill = lu.nnz;
          } else {
            info.fallbacks++;
            current = fallback(a, J);
          }
          info.repivots = lu.repivots - trialRepivots;
        },
        solve(rhs, out) { return scaledSolve(current, rhs, out); },
      };
    };
    if (mode === 'refactor' || mode === 'auto') {
      ...
      // wins where that order fills in (the canister model: 5,800
      // multiply-adds against 45,600); the second where the matrix is banded
      // blocks that order keeps together (a decay chain in a column of cells:
      // 22,800 against 44,000). The cheaper is kept, if it costs less than
      // what auto would otherwise use: n^3/3 for the dense LU (a quarter of
      // it, for margin), or the searching LU's own count, both structural. A
      // trial stops as soon as it has spent that much.
      ...
      const byColumns = trial(q || Int32Array.from({ length: n }, (_, i) => i), byMarkowitz ? Math.min(limit, byMarkowitz.ops) : limit);
    ...
    return {
      info,
      form(a, current, w) { keepScale(w); sparseForm(a, current); },
  ...
  function ndf(f, t0, tfinal, y0, opts = {}) {
    ...
    */
    ...
    return {
      ...
      stats: { nsteps, nfailed, nfevals, npds, ndecomps, nsolves, nbelowtol, negative, sparse: iter.info.sparse, fill: iter.info.fill, ordering: iter.info.ordering, lu: iter.info.lu, repivots: iter.info.repivots || 0, fallbacks: iter.info.fallbacks || 0, solver: bdf ? 'BDF' : 'NDF' },
    };
  }
